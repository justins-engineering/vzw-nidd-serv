...
static void print_token_substring(const char *json_ptr, jsmntok_t *token) {
  if (token->type == JSMN_STRING) {
    printf("Token: %.*s", token->end - token->start, json_ptr + token->start);
    ...
    case 'n':
      ...
      break;
    case 't':
      printf("Token is TRUE");
      break;
    case 'f':
      printf("Token is FALSE");
      break;
    default:
      printf("Token: %d", atoi(json_ptr + token->start));
      break;
    }
  } else {
    printf("Token: %.*s, size: %d, type: %d", token->end - token->start, json_ptr + token->start,
...
static void parse_departures(
  ...
  for (int dep_num = 0; dep_num < departures_count; dep_num++) {
    ...
    for (int dep = 0; dep < (departure_size * 2); dep++) {
      if (jsoneq(json_ptr, &DEPARTURE_TOK, "DisplayText")) {
        ...
        dep++;
        printf("Unexpected key in Departures: %.*s\n", DEPARTURE_TOK.end - DEPARTURE_TOK.start,
  ...
  for (int i = 0; i < MAX_DEPARTURES; i++) {
    printf("Uniq display text(s) %d: %s", i, route_direction->departures[i].display_text);
  }
#endif
  ...
  for (int rd_num = 0; rd_num < route_directions_count; rd_num++) {
    ...
    for (int rdir = 0; rdir < (route_direction_size * 2); rdir++) {
      if (jsoneq(json_ptr, &ROUTE_DIRECTION_TOK, "Direction")) {
        ...
        rdir++;
        printf("Unexpected key in RouteDirections: %.*s\n",
...
int parse_json_for_stop(char *json_ptr, Stop *stop) {
  ...
  switch (ret) {
    ...
    case JSMN_ERROR_PART:
      printf("Failed to parse JSON; The string is not a full JSON packet, more bytes expected.");
      return EXIT_FAILURE;
    default:
      break;
  }
  ...
  while (t < ret) {
    if (jsoneq(json_ptr, &tokens[t], "LastUpdated")) {
      ...
      t++;
      printf("Unexpected key in Stop: %.*s\n", tokens[t].end - tokens[t].start,
